...
| Prisma  | Manifesto manual explícito, `prismaSource`, `PrismaAdapter`, compilation de filtros, search, relações `one`/`many` (`is`, `some`, `none`), select recursivo, sort, paginação, count e `customize`; o perfil estrito usa folded fields e nunca emite `mode: 'insensitive'`. **Corpus de paridade verde em SQLite contra client gerado de verdade**, no mesmo runner do TypeORM e do Drizzle.                                                                  | Generator a partir de `schema.prisma` (o manifesto segue escrito à mão); só SQLite foi exercitado; suíte de compatibilidade Prisma `6.19.x`; e uma divergência documentada: `%` e `_` são coringas em `like`, porque o Prisma não emite cláusula `ESCAPE` e o client tipado não permite fornecê-la. |
| Drizzle | Executor real sobre `drizzle-orm` (`drizzleDatabase()`), relações declaradas por path pontuado (`company`, `company.owner`, `posts`), planner de junções idempotente, `EXISTS` correlacionado para qualquer salto `many`, junção de predicado separada da de apresentação, coleção hidratada por consulta própria, count só com as junções de predicado; MySQL e MSSQL não usam `ILIKE`. **Corpus de paridade verde em SQLite**, no mesmo runner do TypeORM. | Drizzle `1.x` ainda está em RC (`1.0.0-rc.4` nesta branch), não GA; só SQLite foi exercitado — PostgreSQL, MySQL e MSSQL continuam sem rodar; coleção aninhada sob outra relação falha fechado.                                                                                                     |
...
- Drizzle nas três células reais: hoje só o dialeto de referência (SQLite)
  roda o corpus. `OFFSET ... FETCH` do SQL Server e booleano nativo do
  PostgreSQL são testados no SQL compilado, não contra servidor.
- Hidratação de coleção aninhada sob outra relação no Drizzle; hoje falha
  fechado, em vez de devolver a coleção vazia.
- Padrão literal em `like` no Prisma: hoje é divergência documentada, não
  paridade. Enquanto não houver forma de emitir `ESCAPE` pelo client tipado, a
  §11 não vale para o adapter Prisma.
