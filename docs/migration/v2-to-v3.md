...
| Drizzle | Source tipada por tabela/relações explícitas, schema derivado de colunas, compiler/executor por statement abstrato, filtros/search/relação existencial/projeção/sort/count/paginação/customize; MySQL e MSSQL não usam `ILIKE`.                                            | Drizzle `1.x` ainda está em RC (`1.0.0-rc.4` nesta branch), não GA; executor SQL real por dialeto, integração PostgreSQL/MySQL, MSSQL estável e peer `mssql` compatível com a linha Drizzle 1.x. |
