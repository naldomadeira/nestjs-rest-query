# nestjs-rest-query 2.x guide

Guidance for projects on the **2.x** line (`latest` on npm, `2.1.0`). Reached
from `SKILL.md` after version detection said 2.x. Everything below fails on
3.x: `forRoot({ adapter })` is refused at startup there and `RulesConfig` does
not exist. For a 2.x → 3.x upgrade, follow
[`../v3/migrating-from-v2.md`](../v3/migrating-from-v2.md).

**Requires:** NestJS `^11`, Node `>= 20`, and one ORM: TypeORM `^0.3.26`,
Drizzle `^0.45`, or Prisma Client `^5 || ^6 || ^7`.
...
→ Full setup details: [references/setup-reference.md](./setup-reference.md)
...
→ Full operator reference with examples: [references/operators-reference.md](./operators-reference.md)
...
→ Full whitelist reference: [references/rules-reference.md](./rules-reference.md)
...
→ All 8 customize patterns with examples: [references/advanced-patterns.md](./advanced-patterns.md)
→ Common errors and fixes: [references/troubleshooting.md](./troubleshooting.md)
