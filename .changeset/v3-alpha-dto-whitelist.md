...
- When `class-validator` is installed, `page`, `perPage`, `paginate`, `sort`, `fields`, `includes`, `filter` and `search` are now marked with `@Allow()`, so the whitelist keeps them (nested `filter` included). `class-validator` is still not a dependency of this package: it is loaded lazily, and nothing happens when it is absent. This applies to the CommonJS build, which is how Nest apps load the package.
