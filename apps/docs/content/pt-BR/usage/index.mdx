...
- **Parâmetro fora desta tabela é `400`**, não um no-op silencioso:
  `QUERY_SYNTAX_UNKNOWN_PARAM`, com o nome da chave em `details.param` e nunca
  o valor. Um `?utm_source=`, um `?_=` de cache-buster, um `?lang=` de
  middleware — todos recusados. Tire-os antes de a query chegar à DTO.
...
Com `paginate=false` a resposta é `{ "data": [...] }` e nada mais — mas **com
teto**: no máximo `pagination.maxUnpaginatedRows` linhas (default: o
`maxPerPage` efetivo). Um resultado maior é `400 PAGINATION_INVALID`, nunca uma
lista truncada, e um endpoint pode proibir `paginate=false`. Veja
[Paginação](/pt-BR/docs/usage/endpoint-whitelist-rules#paginação). O `lastPage`
...
| `QUERY_SYNTAX_UNKNOWN_PARAM`   | 400        | parâmetro de query fora da gramática de oito nomes; `details.param` é a chave, nunca o valor                                                                                              |
...
| `PAGINATION_INVALID`           | 400        | `page`, `perPage` ou `paginate` não é literal válido, ou excede o `maxPerPage`; `paginate=false` é proibido no endpoint, ou o resultado passa do `maxUnpaginatedRows`                     |
| `SORT_CONFLICT`                | 400        | o mesmo path foi pedido ascendente e descendente                                                                                                                                          |
| `CAPABILITY_UNAVAILABLE`       | 400 ou 500 | o operador ou a forma não pode ser atendida: sem coluna dobrada, sem ordem portável, padrões do Prisma em `sqlite`/`sqlserver` (400); chave primária sem ordem portável (500)             |
| `PORTABILITY_PROFILE_MISMATCH` | 500        | `portability.enforce` ligado e os fatos do perfil da source não batem                                                                                                                     |
| `SOURCE_CONFIGURATION_INVALID` | 500        | o schema declarado não bate com a source, ou o `forRoot` recebeu chave removida ou valor que nenhum adapter implementa (`textProfile: 'database-native'`, `consistency: 'transactional'`) |
