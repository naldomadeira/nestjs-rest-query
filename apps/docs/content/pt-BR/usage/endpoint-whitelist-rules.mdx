...
- O adapter busca no máximo `maxUnpaginatedRows + 1` roots. Uma linha acima do
  teto torna a requisição `400 PAGINATION_INVALID` com
  `details: { param: 'paginate', maxRows }` — a lista nunca é cortada em
  silêncio.
- `allowUnpaginated: false` é `400 PAGINATION_INVALID` antes de qualquer query,
  e o Swagger do endpoint deixa de documentar `paginate`.
