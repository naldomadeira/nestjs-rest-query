...
<Callout type="warn" title="Estas páginas descrevem a v3, publicada como prerelease">

A API `3.x` descrita neste site está no npm como `3.0.0-alpha.0`, sob a tag
`alpha` — `pnpm add nestjs-rest-query@alpha`. A tag `latest` continua apontando
para a `2.1.0`, então uma instalação sem a tag entrega a API `2.x`. A `3.0.0`
estável depende da validação deste alpha e de uma varredura de segurança datada
sobre código v3; os gates que faltam estão em
