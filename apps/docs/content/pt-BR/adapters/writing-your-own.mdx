...
1. **Fez o parse** da entrada contra um alfabeto seguro de paths. `*`, `;`,
   ...
   projeção fora do alcance do cliente. (`QUERY_SYNTAX_UNKNOWN_PARAM` está no
   contrato de erros mas não é emitido hoje: um parâmetro de query que a
   gramática não conhece é ignorado, não recusado.)
