...
O `drizzleSource` fixa o tipo da linha em `object`; só o `typeormSource` é
genérico no row, então anote `Promise<NormalizedQueryResult<object>>` em vez de
usar cast.
...
de `columns` por `sql.identifier(...)`; o campo `name` do `DrizzleColumn` é
declarado e nunca lido. Uma chave de descritor diferente do nome físico da
coluna compila, passa na verificação da source, sobe — e então falha no banco
com `column "companyId" does not exist`. Mantenha chave e nome físico
idênticos.

É por isso que o
[`03-app-with-drizzle`](https://github.com/naldomadeira/nestjs-rest-query/tree/main/apps/examples/03-app-with-drizzle)
nomeia as colunas físicas em camelCase (`"companyId"`, `"createdAt"`) e adiciona
uma verificação na subida contra `getTableColumns(pgTable)` para transformar a
divergência em erro de inicialização. É contorno de lacuna da biblioteca, não o
desenho pretendido.
