---
title: Introdução
description: O que é o NestJS Dynamic Query Builder e por que utilizá-lo.
---

**NestJS Dynamic Query Builder** é uma biblioteca para NestJS que constrói instâncias de `SelectQueryBuilder` do TypeORM de forma dinâmica a partir de parâmetros de query HTTP — filtros, ordenações, paginação, seleção de campos e carregamento de relações — tudo controlado por uma whitelist de segurança por endpoint.
...
flowchart TD
  ...
  subgraph LIB["nestjs-dynamic-query-builder"]
...
<Cards>
  <Card title="Pré-requisito NestJS" href="/docs/getting-started/prerequisites" />
