...
1. **Parsed** the input against a safe path alphabet. `*`, `;`, `..`, an empty
   ...
   client's reach. A parameter outside the eight-name grammar (`filter`, `sort`,
   `fields`, `includes`, `search`, `page`, `perPage`, `paginate`) is refused
   with `400 QUERY_SYNTAX_UNKNOWN_PARAM`, never ignored; the key name travels in
   `details`, the value never does.
