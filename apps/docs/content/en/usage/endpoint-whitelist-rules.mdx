...
| `perPage`  | positive integer | `10`    | Above `maxPerPage` (default `100`) it is a `400`. |
...
- The adapter fetches at most `maxUnpaginatedRows + 1` roots. One row over the
  cap makes the request a `400 PAGINATION_INVALID` with
  `details: { param: 'paginate', maxRows }` — the list is never silently cut.
- `allowUnpaginated: false` is a `400 PAGINATION_INVALID` before any query runs,
  and the endpoint's Swagger stops documenting `paginate`.
