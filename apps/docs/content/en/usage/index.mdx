...
but it is **capped**: at most `pagination.maxUnpaginatedRows` rows (default: the
effective `maxPerPage`). A larger result is a `400 PAGINATION_INVALID`, never a
truncated list, and an endpoint can forbid `paginate=false` altogether. See
...
| `PAGINATION_INVALID`           | 400        | `page`, `perPage` or `paginate` is not a valid literal, or exceeds `maxPerPage`; `paginate=false` is forbidden on the endpoint, or its result exceeds `maxUnpaginatedRows`        |
