...
<Callout type="warn" title="These pages describe v3, published as a prerelease">

The `3.x` API described on this site is on npm as `3.0.0-alpha.0`, under the
`alpha` tag — `pnpm add nestjs-rest-query@alpha`. The `latest` tag still points
at `2.1.0`, so an install without the tag gives you the `2.x` API. Stable
`3.0.0` waits on validation of this alpha and on a security scan dated over v3
code; the remaining gates are listed in
